...
namespace PeskyBox.MediaToolkitManager
{
    ...
    {
        [Tooltip("Number of decibels to boost the mic holder by when used")] [Range(0, 9)]
        ...
        [Tooltip(
            "Radius in which to muffle people. Everyone becomes progressively quieter depending on how close the local player is to the microphone.")]
        public float MuffleRange = 10f;


        //State settings
        [UdonSynced(UdonSyncMode.None)] private bool poweredOn = false;

        [UdonSynced(UdonSyncMode.None)] private int MicHolderId = -1;
